...
| `agentic.longestTurnMs`                            | number \| null              | active length of the longest turn: sum of gaps between the turn's records, gaps over 30 min excluded (an open idle session is not a long turn)                    |
| `outcome.prLinks`                                  | number                      | distinct PRs the harness linked to the session (count only, deduped by PR identity)                                                                               |
